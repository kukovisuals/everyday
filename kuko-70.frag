...
vec4 map(vec2 p)
{
    ...
    // rotation
    float r_speed = 0.4;
    p_r1 = rotate2D(u_time * (r_speed)) * p_r1;
    p_r2 = rotate2D(u_time * (r_speed/2.0)) * p_r2;
    p_r3 = rotate2D(u_time * (r_speed/3.0)) * p_r3;
    p_r4 = rotate2D(u_time * (r_speed/4.0)) * p_r4;
    // location 
    vec2 shape[ARR_SIZE];
...
vec3 noiseColor(vec2 uv)
{
    ...
    float n_sun = fbm11(uv * 0.9 + u_time * 0.0329); 
    
    new_uv.x = sin(uv.x * 0.2 + u_time * 0.02) * 0.5 + n * 0.23923;
    
    float xRepeat = mod(new_uv.x * 0.9, 0.4);
    float line    = smoothstep(0.0, 0.1, xRepeat) - smoothstep(0.2 - 0.01, 0.2, xRepeat);

    float sun = sdfCircle(uv, 0.33) + n_sun * 0.2;
