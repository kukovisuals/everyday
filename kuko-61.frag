...
{ 
    float fx = sin(u_time * 0.6) * 0.7 + 1.0;
    return length(p + vec3(0.0, 0.0, 1.4 - fx)) - r;
}
mat2 rotate2D(float a){ return mat2(cos(a), -sin(a), sin(a), cos(a));}
float map(vec3 p){
    vec3 new_p = p;
    float theta_fx = -PI/8.0 * (sin(u_time * 0.3) * 0.5 + 0.5);
...
{
    float color_fx = smoothstep(0.0,1.0, (norm.y) +  0.5) ;
    float hue = color_fx * 0.333 + 0.353;
