...
float sphere(vec3 p)
{
    ...
    //     1.0 - 0.05;
    float d = length(
        cos(p) + p.x * 0.15315 * p.y * 
        0.0635315*sin(5.0*(1.08 * p.x + u_time * 0.1))) - 
        1.08080 - 0.0;
    return d;
}
...
float map(vec3 p)
{
    int dtFx = int(mod(u_time / 2.0, 6.0));
    if(dtFx == 0 ){
        p = rotateX(u_time * 0.00002) * p;
    } else if(dtFx == 1 ){
        p = rotateZ(u_time * -0.02) * p;
    } else if(dtFx == 2 ){
        p = rotateX(u_time * 0.0002) * p;
    } else if(dtFx == 3 ){
        p = rotateZ(u_time * 0.002) * p;
    } else if(dtFx == 4 ){
        p = rotateX(u_time * 0.0001) * p;
    } else {
        p = rotateY(u_time * 0.0002) * p;
    }
    // p = rotateZ(u_time * 0.3) * p;
    float spxTime = sin(p.y * 0.002716) * 0.15 + 0.5;
    return sphere(p - vec3(3.0, 3.0, -1.0));
    // return sdBox(
...
void mainImage(out vec4 O, in vec2 I)
{
    ...
    float cmrSpeed = 1.0;
    int timeFx = int(mod(u_time / 2.0, 6.0));
    ...
    if(t < 15.0)
    {
        ...
        int dtFx = int(mod(u_time / 2.0, 2.0));
        
        float ringDist = length(p.xy - vec2(0.0, -7.0)); 
        float rings = tan(ringDist * 5.90 + u_time * 1.5);

        if(dtFx == 0 ){
            rings = tan(ringDist * 1.90 + u_time * 0.5);
        } else {
            rings = tan(ringDist * 5.90 + u_time * 1.5);
        }
    ...
    // color = vec3(uv.x);
    // O = vec4(color, 1.0);
    float ldt = sin(u_time * 0.5) * 3.0 + 3.6;
