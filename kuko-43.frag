...
{
    float sinFx = sin(u_time * 10.0) * 0.1 + 0.1;
...
//     vec2 newUv = fract(uv * 1.5) - 0.5;
    
//     vec3 color = vec3(0.0);

//     const int SHAPE_COUNT = 15;
//     vec2 positions[SHAPE_COUNT] = vec2[](
//         vec2(-0.049,  0.054), vec2(-0.125, -0.065), vec2(-0.3141, -0.294), vec2( 0.122,  0.330), vec2( 0.2370, -0.044),
//     vec2(-0.31435,  0.1350), vec2(-0.23410,  0.24315), vec2( 0.050,  0.322), vec2( 0.3128,  0.412), vec2( 0.2440,  0.160),
//     vec2(-0.148,  0.350), vec2(-0.293430, -0.041), vec2(-0.120, -0.2404), vec2( 0.2410, -0.2051), vec2( 0.31431, -0.2308)
//     );

//     for(int i=0; i<SHAPE_COUNT; i++)
//     {
//        float m2 = opOnion(newUv - (positions[i]) , 0.001);
//         m2 = smoothstep(0.01, 0.0, m2);
//         color += vec3(m2);
//     }
