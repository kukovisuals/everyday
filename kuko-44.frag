...
// // Alternative: straight sine waves that bend around circles
// float bendingSineWaves(vec2 uv, vec2 center, float time) {
//     vec2 toCenter = uv - center;
//     float dist = length(toCenter);
//     float angle = atan(toCenter.y, toCenter.x);
    
//     // Create sine waves that bend around the circle
//     float bendFactor = max(0.0, 1.0 - dist * 2.0); // Stronger bend closer to circle
    
//     // Original sine wave position
//     float sinePos = uv.x * 2.0 + time * 3.0;
    
//     // Bend the sine wave around the circle
//     sinePos += angle * bendFactor * 2.0;
    
//     float wave = sin(sinePos) * 0.03 * (1.0 + bendFactor);
//     float waveDist = abs(uv.y - center.y - wave);
    
//     // Create flowing lines
//     float lineIntensity = 1.0 - smoothstep(0.0, 0.005, waveDist);
    
//     // Add multiple waves
//     float wave2 = sin(sinePos * 1.3 + 1.0) * 0.025 * (1.0 + bendFactor);
//     float waveDist2 = abs(uv.y - center.y - 0.1 - wave2);
//     lineIntensity += (1.0 - smoothstep(0.0, 0.005, waveDist2)) * 0.7;
    
//     float wave3 = sin(sinePos * 0.8 + 2.0) * 0.035 * (1.0 + bendFactor);
//     float waveDist3 = abs(uv.y - center.y + 0.1 - wave3);
//     lineIntensity += (1.0 - smoothstep(0.0, 0.005, waveDist3)) * 0.5;
...
// {
//     vec2 uv = (2.0 * I - u_resolution) / u_resolution.y;
//     float time = u_time; // Assuming you have time uniform
...
//     // Add flowing waves around both circles
//     vec2 leftCenter = -left_eye_loc;  // Convert to world space
//     vec2 rightCenter = -right_eye_loc;
    
//     // Method 1: Circular flowing waves
//     float leftWaves = flowingWaves(new_uv, leftCenter, time);
//     float rightWaves = flowingWaves(new_uv, rightCenter, time);
    
//     // Method 2: Bending sine waves (uncomment to use instead)
//     // float leftWaves = bendingSineWaves(new_uv, leftCenter, time);
//     // float rightWaves = bendingSineWaves(new_uv, rightCenter, time);
    
//     // Combine wave effects
//     float totalWaves = leftWaves;
    
//     // Add waves to the final color
//     color *= mix(white, waveColor, leftWaves * 0.6);
//     color *= mix(white, red, rightWaves * 0.6);
    
//     // Optional: Add some randomness to the waves
//     float noise = fract(sin(dot(new_uv, vec2(12.9898, 78.233))) * 43758.5453);
//     totalWaves *= (0.8 + noise * 0.4);
...
//     vec2 new_uv = fract(uv) - 0.5;
//     vec2 left_eye_loc = vec2(0.25, -0.25);
//     // float left_eye = length(new_uv + left_eye_loc) - 0.15;
//     float left_eye = sdCircle(new_uv, left_eye_loc, 0.15);
//     left_eye = smoothstep(0.0, 0.1, left_eye);
    
//     color *= mix(red, white, left_eye);

//     vec2 right_eye_loc = vec2(-0.25, -0.25);
//     // float right_eye = length(new_uv + right_eye_loc) - 0.15;
//     float right_eye = sdCircle(new_uv, right_eye_loc, 0.15);
//     right_eye = smoothstep(0.0, 0.1, right_eye);
//     color *= mix(red, white, right_eye);
