...
uniform sampler1D sinCosTex; // Pre-computed sin/cos values
...
// Fast approximation of sin(x) where x is in [0, 2π]
float fastSin(float x) {
    // Wrap to range [0, 2π]
    x = mod(x, 6.28318530718);
    
    // Transform to [-1, 1]
    float y = x * 0.318309886184 - 1.0; // x/π - 1
    
    // Calculate approximate sine using polynomial
    float y2 = y * y;
    return -0.4 * y * (y2 - 1.0) * (y2 - 4.0);
}

// Fast approximation of cos(x) = fastSin(x + π/2)
float fastCos(float x) {
    return fastSin(x + 1.57079632679);
}
...
vec2 getLCoords(vec2 p, vec2 cellCenter, vec2 cellId) 
{
    ...
    float randScaleX = 3.0 + 1.0 * H(cellId).y;
