...
{
    vec3 new_p = p; 
    new_p.zx = rotate2D(u_time) * new_p.zx;
...
float map(vec3 p)
{
    ...
    float d1_fx_two = pow(sin(u_time * speed + 3.0) * 1.0 + 0.8, 3.0);
    float d1 = sdSphere(p + vec3(0.0,0.0, 0.5 + d1_fx), 2.0);
    float d3 = sdSphere(p + vec3(-3.0,-3.0, 0.5 + d1_fx_two ), 2.0);
    float d4 = sdSphere(p + vec3( 3.0,-3.0, 0.5 + d1_fx_two), 2.0);
    float d5 = sdSphere(p + vec3(-3.0, 3.0, 0.5 + d1_fx_two), 2.0);
    float d6 = sdSphere(p + vec3( 3.0, 3.0, 0.5 + d1_fx_two), 2.0);
    float d2 = p.z + 1.0;
    // Combine all spheres first
    float spheres = smin(d1, d3, 3.0);
    spheres = smin(spheres, d4, 3.0);
    spheres = smin(spheres, d5, 3.0);
    spheres = smin(spheres, d6, 3.0);
...
void mainImage(out vec4 O, in vec2 I)
{
    ...
    uv.xy *= rotate2D(u_time * 2.3) * uv.xy;
