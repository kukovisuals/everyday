...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float dt = u_time;
    vec2 uv = 3.5 * (fragCoord + fragCoord - iResolution.xy) / iResolution.y;
    uv.y += dt * 0.1;
    
    float scale = 1.0;
    vec3 d1 = voronoiDistSq(uv * scale, dt, vec3(9.0), false );
    ...
    vec3 d2 = voronoiDistSq(localCoor + vec2(cellId.x * 0.1, cellId.y * 0.1), dt * 1.5, vec3(9.0), true);
