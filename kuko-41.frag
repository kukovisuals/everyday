...
float map(vec3 p)
{
    ...
    handPos.y += sin(handPos.x * 4.0 * 1.0 * 3.14159 + u_time * 2.0) * 0.08;  // 8 complete cycles
    handPos.z += cos(handPos.x * 2.0 * 1.0 * 3.14159 + u_time * 1.5) * 0.04; // 6 complete cycles
...
void mainImage(out vec4 O, in vec2 I)
{
    ...
    // color = vec3(distT * 0.05 + float(i) * 0.005);
