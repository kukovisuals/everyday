...
{
    float spiralW = 0.5;
    p.xy *= R2(p.z * spiralW + T * 0.00001);
    p.z -= T * 1.0;
    
    vec3 ghostUv = p + vec3(0, -2.5, 0);
    //ghostUv.z += T * 0.5;
    ghostUv.x -= 1.05;
    ...
    float fxY = sin(T * jumpSpeed + timeOffset) * 0.5 + 0.5;
    
    ghostUv = vec3(
        mod(ghostUv.x, 4.0) - 2.0, 
        ghostUv.y + 2.0 + fxY, 
        mod(ghostUv.z + 0.0, 5.0) - 2.5
    );
    
    float punchOff = hash(ghostCellId + 200.0) * 0.40;
    float punchDt  = 0.008 + hash(ghostCellId + 300.0) * 0.28;
    float punchT   = T * punchDt + punchOff;
    float punchRot = exp(mod(punchT, 4.0)) * sin(mod(punchT, 4.0) * 2.0);
    
    ghostUv.xz *= R2(punchRot);
    
    return sdfLittleGhost(ghostUv);
}
...
//     float punchOff = hash(ghostCellId + 200.0) * 0.40;  // 0-8 second offset
//     float punchDt = 0.008 + hash(ghostCellId + 300.0) * 0.28; // Speed variation: 1.8-2.6
//     float punchT = T * punchDt + punchOff;
//     float punchRot = exp( mod(punchT, 4.0)) * sin(mod(punchT, 4.0) * 2.0);
...
vec3 render(vec3 ro, vec3 rd, float T)
{
    ...
    if(dt.y > -0.0)
    {
        ...
        if(dt.y >  9.9){
            ...
            // eye shadow
            bgCol = vec3(0) * 0.8; 
            diffC = diff * 0.4 * lightCol * 0.5;
        } else if(dt.y >  6.9){
            // body little ghost
            float gPos = sdfLittleGhost(p).x;
            float caustic = sin(gPos * 1.0 + T * 1.5);
            caustic *= sin(dot(norm, -rd) * 3.0 + T * 0.1);
            vec3 ris  = refract(norm, tangent, 1.0) ;
            float rl = cos(dot(vec3(caustic), vec3(0,0,1)) * 1. + T * 1.1) * 2.5 + 0.5;
            
            vec3 albedo = palette(abs(rl), vec3(0.7), vec3(0.8), vec3(0.25), vec3(.17, 0.35, 0.4));
            bgCol = albedo; 
            diffC = rim * 3.1 * diff  * 0.5 * lightCol * 0.9;
            
        } else {
            bgCol = vec3(0.812,0.396,0.008) * 1.4; 
            diffC = rim * diff * 100.5 * lightCol * 1.0;
        }
...
void mainImage(out vec4 O, in vec2 I)
{
    ...
    vec3 ta = vec3(0,0,3); 
    vec3 ro = ta; //+ vec3(6.0*cos(0.0 * T), 0, 6.0*sin(0.0 * T)); 
    ...
    for (int n = 0; n < AA; n++) {
        ...
        vec2 uv = (2.0 * (I + off) - u_resolution.xy) / u_resolution.y;
        // uv *= .34;
