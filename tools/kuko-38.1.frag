...
float map(vec3 p)   
{
    ...
    float spxTime = sin(u_time * 0.5) * 0.3 + 2.1;
    return sphere(p - vec3(0.0, 0.7, -1.0));
    // return sdBox(
...
void mainImage(out vec4 O, in vec2 I)
{
    ...
    vec3 ta = vec3(0.0, 0.0, -2.0 - dt * 1.0); 
    ...
    if(t < 60.0)
    {
        ...
        // );
        float wave = sin(p.x * 5.0 + u_time * 4.) * cos(p.z * 20.0 + u_time * 1.1);
        baseColor = vec3(
            0.3 + 0.7 * wave,
            0.8 + 0.1 * wave,
            0.8 - 0.1 * wave
    ...
    O = vec4(3.5 * color * exp(-t/(ldt)), 1.0);
}