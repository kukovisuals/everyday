...
void mainImage(out vec4 O, in vec2 I)
{
    ...
    float lineD = drawLine(uv, B, D, lineThickness);     // Origin to D (blue)
    float lineE = drawLine(uv, B, E, lineThickness);     // Origin to E (purple)
    
    // Optional: Draw projection lines (perpendicular from A to the projected points)
